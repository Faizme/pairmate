...
export const PartnerCard = ({ 
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      let tsMs = Date.now();
      if (partnerLastActive.seconds) { // Firestore Timestamp
         tsMs = partnerLastActive.seconds * 1000;
      } else if (partnerLastActive._seconds) { 
         tsMs = partnerLastActive._seconds * 1000;
      } else { // ISO String fallback
         tsMs = new Date(partnerLastActive).getTime();
      }
      ...
      if (diffSecs < 30) {
        ...
        setTimeText(`Last active ${mins} min ago`);
      } else if (diffSecs < 86400) {
        const hours = Math.floor(diffSecs / 3600);
        setTimeText(`Last active ${hours} hr ago`);
      } else {
        setTimeText('Last active a while ago');
      }
    };
       ...
       {partnerName ? (
         <>
           <div className="flex flex-col items-center mt-6">
               <h2 className="text-2xl font-light tracking-wide text-white">{partnerName}</h2>
               {timeText && <p className="text-[10px] mt-2 text-white/30 tracking-widest">{timeText}</p>}
           </div>
